...
- **Sector Intelligence**: Granular analysis isolated by track sectors.
...
2. **Vision Inference**: Each frame is evaluated by CLIP against highly descriptive labels (e.g. `"a completely dry racing track surface in sunny weather"`).
...
4. **Strategy Formulation**: Based on the trend slope and current condition, a rule-based engine outputs a `DECISION` (MONITOR, PREPARE, HOLD).
